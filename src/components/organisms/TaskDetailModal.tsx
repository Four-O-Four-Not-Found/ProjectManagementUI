    ...
    <div className="space-y-1">
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1 px-1.5 py-0.5 rounded bg-surface border border-border">
          {task.type === 'Issue' && <AlertCircle size={10} className="text-rose-500" />}
          {task.type === 'Suggestion' && <Lightbulb size={10} className="text-amber-500" />}
          {task.type === 'Feature' && <CheckCircle2 size={10} className="text-primary" />}
          <span className="text-[9px] font-bold text-text-muted uppercase tracking-widest">{task.type}</span>
        </div>
      </div>
      <h2 className="text-lg font-bold text-text-main tracking-tight">{task.title}</h2>
    </div>
  );
    ...
    >
      <div className="flex flex-col md:flex-row min-h-[500px]">
          ...
          <div className="flex gap-6 border-b border-border mb-6">
            {(['details', 'activity', 'attachments'] as const).map((tab) => (
              ...
              >
                {tab}
                {activeTab === tab && (
                  <motion.div layoutId="activeTabDetail" className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary shadow-[0_0_8px_rgba(56,189,248,0.4)]" />
                )}
              </button>
            ))}
          </div>
          ...
          {activeTab === 'details' && (
            <div className="space-y-6 animate-fade-in">
              <div className="prose prose-invert max-w-none">
                <h3 className="text-[10px] font-bold text-text-muted uppercase tracking-widest mb-3">Description</h3>
              ...
              <div>
                <h3 className="text-[10px] font-bold text-text-muted uppercase tracking-widest mb-3">Visual Evidence / Attachments</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {task.attachments?.filter(a => a.type === 'image').map((img) => (
                    <div key={img.id} className="aspect-video rounded-xl overflow-hidden border border-border group relative cursor-pointer">
                  ...
                  <button className="aspect-video rounded-xl border-2 border-dashed border-border hover:border-primary/50 hover:bg-primary/5 transition-all flex flex-col items-center justify-center gap-2 group">
                    <ImageIcon size={18} className="text-text-muted group-hover:text-primary transition-colors" />
                    <span className="text-[9px] font-bold text-text-muted uppercase group-hover:text-primary transition-colors">Add Image</span>
                  </button>
                </div>
              </div>
            </div>
                ...
                <div key={comment.id} className="flex gap-3">
                  ...
                  <div className="flex-1 bg-surface-hover/20 rounded-xl p-3 border border-border">
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-xs font-bold text-text-main">{comment.userName}</span>
                      <span className="text-[9px] text-text-muted">{comment.timestamp}</span>
                    </div>
              ...
              <div className="flex gap-3 mt-6">
                ...
                <div className="flex-1 relative">
                  ...
                  <div className="absolute bottom-2 right-2 flex items-center gap-2">
                    ...
                    <Button size="xs" className="h-8 px-3" rightIcon={<Send size={14} />}>
                      Post
                    </Button>
                  </div>
                </div>
        ...
        <div className="w-full md:w-64 bg-surface-hover/30 border-l border-border p-4 md:p-6 space-y-6">
          <div>
            <h3 className="text-[9px] font-bold text-text-muted uppercase tracking-widest mb-3">Properties</h3>
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-text-muted">Status</span>
                <Badge variant="primary" size="xs">{task.status}</Badge>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-text-muted">Priority</span>
                <Badge variant={task.priority === 'Urgent' ? 'danger' : 'warning'} size="xs">{task.priority}</Badge>
              </div>
              <div className="flex items-center justify-between">
          ...
          <div>
            <h3 className="text-[9px] font-bold text-text-muted uppercase tracking-widest mb-3">Assignee</h3>
            <div className="flex items-center gap-3 bg-background p-2.5 rounded-xl border border-border">
              ...
              <div className="min-w-0">
                <p className="text-[11px] font-bold text-text-main truncate">{task.assignee?.name || 'Unassigned'}</p>
                <p className="text-[9px] text-text-muted truncate">Core Team</p>
              </div>
            </div>
          </div>

          <div>
            <h3 className="text-[9px] font-bold text-text-muted uppercase tracking-widest mb-3">Timeline</h3>
