...
import { MessageSquare, Paperclip, GitBranch, ArrowRightLeft, Lightbulb, AlertCircle, Bookmark } from 'lucide-react';
...
const TypeIcon = ({ type }: { type: Task['type'] }) => {
  switch (type) {
    case 'Issue': return <AlertCircle size={12} className="text-rose-400" />;
    case 'Suggestion': return <Lightbulb size={12} className="text-amber-400" />;
    case 'Bug': return <AlertCircle size={12} className="text-rose-500" />;
    default: return <Bookmark size={12} className="text-primary" />;
  }
};

const PriorityBadge = ({ priority }: { priority: Task['priority'] }) => {
  const variants: Record<Task['priority'], "secondary" | "primary" | "warning" | "danger"> = {
    Low: 'secondary',
    Medium: 'primary',
    High: 'warning',
    Urgent: 'danger',
  };
  return <Badge variant={variants[priority]} size="xs">{priority}</Badge>;
};

const TaskCard: React.FC<TaskCardProps> = ({ task, onClick, onMove, className }) => {
    ...
    >
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-3 text-text-muted">
          ...
          <div className="flex items-center gap-1">
            <Paperclip size={12} />
            <span className="text-[10px]">{task.attachments?.length || 0}</span>
          </div>
        </div>
      </div>
    </motion.div>
