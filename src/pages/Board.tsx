...
const Board: React.FC = () => {
  ...
  const handleCreateSprint = (data: Partial<Sprint>) => {
    success('Sprint Launched', `Sprint "${data.name}" is now the active focus.`);
	...
	return (
      ...
      />
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="font-mono text-[10px] text-text-muted bg-surface px-1.5 py-0.5 rounded border border-border uppercase">{currentProject.key}</span>
          </div>
        }
				actions={
      ...
      <div className="flex items-center gap-1 border-b border-border bg-surface-hover/30 px-2 rounded-t-md">
        {(['Board', 'Backlog', 'Timeline', 'Team', 'Settings'] as ViewTab[]).map((tab) => {
          const Icon = {
            Board: Layout,
            Backlog: ListIcon,
          ...
          return (
            ...
            >
              <Icon size={14} />
              {tab}
              {tab === 'Backlog' && <span className="ml-1 px-1.5 py-0.5 rounded-full bg-surface border border-border text-[9px]">{tasks.length}</span>}
            </button>
          );
        })}
      </div>
          ...
          {activeTab === 'Backlog' && (
            ...
            >
              <div className="p-4 bg-surface-hover border-b border-border flex items-center justify-between">
                <span className="text-xs font-bold uppercase text-text-muted">Filtered Issues</span>
                <div className="flex gap-2">
                  <input type="text" placeholder="Filter tasks..." className="bg-background border border-border rounded px-3 py-1.5 text-xs text-text-main outline-none focus:border-primary w-64" />
                </div>
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-background/50">
                {tasks.map(task => (
                  <div key={task.id} className="p-3 bg-surface border border-border rounded hover:border-text-muted transition-colors flex items-center justify-between group cursor-pointer">
                    <div className="flex items-center gap-3">
                      <div className={`w-2 h-2 rounded-full ${task.type === 'Bug' ? 'bg-danger' : task.type === 'Issue' ? 'bg-warning' : 'bg-success'}`} />
            ...
            >
              ...
              <div>
                ...
                <p className="text-sm text-text-muted mt-2 max-w-sm">
                  The interactive timeline view is currently synchronizing with the {currentProject.key} milestones.
                </p>
              </div>
